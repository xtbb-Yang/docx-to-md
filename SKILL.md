---
name: docx-to-md
description: 当用户需要将文档转换为适合 Obsidian、知识库入库、RAG 检索或模型阅读的干净 Markdown 时，应使用此 skill；支持 .docx（Pandoc）、.pdf、.pptx、.xlsx、.html、.htm（MarkItDown）格式；具备单文件转换、批量转换、递归转换、默认图片提取、仅文本转换、Markdown 图片显示与可点击原图链接、内容清理、HTML 表格转 Markdown 表格、图片 alt 文本保留、链接文本提取和内联 HTML 清理等能力。
---
...
| `.html`、`.htm` | MarkItDown (BeautifulSoup + markdownify) | `scripts/convert_other_to_md.py` |
...
- `.pdf`、`.pptx`、`.xlsx`、`.html`、`.htm` 使用 `scripts/convert_other_to_md.py`，基于 MarkItDown 库转换，并经过与 docx 相同的后处理清理管道。
...
### PDF / PPTX / XLSX / HTML 转换依赖

转换 `.pdf`、`.pptx`、`.xlsx`、`.html`、`.htm` 依赖 `markitdown` Python 包（HTML 转换能力随 markitdown 主包自带，依赖 `beautifulsoup4` + `markdownify`）。
...
- `openpyxl`（Excel 解析）
- `beautifulsoup4` + `markdownify`（HTML 解析与转换，随 markitdown 主包自带）
...
python3 scripts/convert_other_to_md.py --input-file page.html --output-file page.md
```

批量转换目录中所有支持的文件（PDF + PPTX + XLSX + HTML）：
...
- `.pdf`、`.pptx`、`.xlsx`、`.html`、`.htm` 文件 → 使用 `scripts/convert_other_to_md.py`
...
转换 `.pdf` 时，以下任一条件成立，应使用 `--vision` 模式（VLM 看图理解），而不是默认的 MarkItDown 文本提取：

**用户提示词触发**（显式或隐式）：
- 显式指令：用户提到"用 vision"、"VLM"、"多模态"、"识图模式"、"看图模式"、"图像识别"、"视觉模式"。
- 文件名提示：文件名包含"scan"、"screenshot"、"扫描"、"截图"。
- 任务暗示：用户说"提取图表数据"、"还原架构图"、"识别海报内容"、"看图理解"、"图表里的具体数字"等。

**文件特征触发**（agent 自行推断）：
- PDF 含大量图表、架构图、流程图、海报、设计稿、UI 截图。
- 默认模式（MarkItDown 文本提取）转换后，产物 md 每页平均文字 < 50 字（疑似扫描件或纯图 PDF），应自动重跑 `--vision`。

**Vision 模式仅对 PDF 生效**，对 PPTX/XLSX 传入 `--vision` 会被忽略。
...
7. **【必须】对 PDF/PPTX 转换产物执行自检修复**（详见下方"产物自检修复"章节）；HTML/HTM 转换质量较高，可跳过自检。

## 产物自检修复（PDF / PPTX 必须执行）

### 触发条件

对所有 `.pdf` 和 `.pptx` 文件的转换产物，**必须**在脚本转换完成后执行此自检步骤。`.docx`、`.xlsx`、`.html`、`.htm` 转换质量较高，可跳过此步骤（除非用户要求）。
...
### convert_other_to_md.py（PDF / PPTX / XLSX / HTML）
...
- 仅适用于本地静态 HTML（AI 生成的讲解/展示页面），典型特征：无 nav/footer/ads 噪声、图片以 base64 内联或相对路径引用、`<style>`/`<script>` 在 `<head>` 中。
- 转换引擎为 MarkItDown 内置的 HTML Converter（BeautifulSoup 解析 body → markdownify 转 Markdown），自动剥离 `<script>`、`<style>` 块。
- 不适合带导航栏/侧边栏/页脚/广告的网页（这些噪声会全部转成 Markdown）。如需此类网页转 Markdown，应使用正文提取方案（如 trafilatura），不在本 skill 范围内。
...
- `--input-file`：单个文件（PDF/PPTX/XLSX/HTML/HTM）。
